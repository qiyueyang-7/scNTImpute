...
from scNTImpute.batch_sampler import CellSampler

_logger = logging.getLogger(__name__)


class BaseCellModel(nn.Module):
    """Base class for single cell models.F
    ...
    def train_step(self,
        ...
        self.train()
        optimizer.zero_grad()
        if shamgmiande_epoch == n_epoch-2:
            loss, drop_loss, fwd_dict, new_record, self_scNTImputeadata_X = self(data_dict, hyper_param_dict, shamgmiande_epoch)
        else:
            loss, drop_loss, fwd_dict, new_record = self(data_dict, hyper_param_dict, n_epoch)
        if loss_update_callback is not None:#loss_update_callback更新损失和记录字典的可调用对象。 =Nnone
            loss, new_record = loss_update_callback(loss, fwd_dict, new_record)
        loss.backward()
        drop_loss.requires_grad_(True)
        drop_loss.backward()
        norms = torch.nn.utils.clip_grad_norm_(self.parameters(), 50)
        new_record['max_norm'] = norms.cpu().numpy()
        optimizer.step()
        if shamgmiande_epoch == n_epoch-2:
            return new_record, self_scNTImputeadata_X
        else:
            return new_record
