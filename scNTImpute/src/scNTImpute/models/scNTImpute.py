...
from torch.distributions import Normal, Independent, Gamma
...
class scNTImpute(BaseCellModel):
        ...
        for i in range(len(f1)):
            ...
            while (np.isnan(recon_log[similar_cell_number, f1[i, 1]])).all():
                similar_cell_number = np.argwhere(cell_distance[number_cells_tobe_interpolated, :]
                                                == min(filter(lambda x : x > min_value, cell_distance[number_cells_tobe_interpolated, :])))
                min_value=min(filter(lambda x : x > min_value, cell_distance[number_cells_tobe_interpolated, :]))
            recon_log[number_cells_tobe_interpolated, f1[i, 1]] = recon_log[similar_cell_number, f1[i, 1]]
        return recon_log
    ...
    def calculate_distance_cells(self,
        ...
        hewei_matrix = torch.from_numpy(np.delete(hewei_matrix, 0, axis=0).astype(float)).cuda()
        return hewei_matrix
    ...
    def forward(self,
        ...
        normed_cells = cells / (library_size+1)
        ...
        genes = cells.T
        normed_genes = torch.log10(genes_1).T
        input_genes = normed_genes if self.norm_genes else genes
        ...
        if not self.training:
            ...
            if 'decode' in hyper_param_dict and hyper_param_dict['decode']:
                if self.n_epoch-2 == n_epoch:
                    recon_log, drop_loss, self_scNTImputeadata_X = self.decode(theta=theta, qi___five=qi___five_softmax,
                                                   genes_cells=normed_genes, batch_indices = data_dict.get('batch_indices', None), n_epoch=n_epoch)
                else:
                    recon_log, drop_loss = self.decode(theta=theta, qi___five=qi___five_softmax,
