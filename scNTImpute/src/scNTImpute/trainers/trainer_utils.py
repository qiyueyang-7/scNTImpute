...
class _stats_recorder:
    ...
    def __init__(self,
        ...
        if record_log_path is not None:
            ...
            # trainer.ckpt_dir=os.path.join(ckpt_dir, f"{self.train_instance_name}_{time.strftime('%m_%d-%H_%M_%S')}")
    ...
    def log_and_clear_record(self) -> None:
        """Logs record to logger and reset self.record.  将记录记录到记录器并重置自我记录"""
...
def prepare_for_transfer(
...
) -> Tuple[scNTImpute, anndata.AnnData]:
    ...
    src_shared_indices = [i for i, gene in enumerate(tgt_genes)]
    shared_genes=list(tgt_genes)
    tgt_model = copy.deepcopy(model)

    tgt_model.n_trainable_cells=tgt_dataset.n_obs
    tgt_model.adata_scNTImpute=tgt_dataset
    tgt_model.batch_size=tgt_dataset.n_obs
    tgt_model.loglik_initial_value=torch.zeros([tgt_dataset.n_vars])

    tgt_model.n_fixed_genes = n_fixed_genes
    tgt_model.n_trainable_genes = tgt_dataset.n_vars - n_fixed_genes

    tgt_model.n_batches= tgt_dataset.obs.batch_indices.nunique()
