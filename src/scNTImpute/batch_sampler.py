...
class CellSampler():
    ...
    Attributes:
        n_cells: number of cells in the
        dataset.
...
class GenesSampler():
    ...
    def __iter__(self) -> Iterator[Mapping[str, torch.Tensor]]:
        ...
        if self.batch_size < self.n_genes:
            return self._low_batch_size()
        else:
            return self._high_batch_size()
    ...
    def _high_batch_size(self) -> Iterator[Mapping[str, torch.Tensor]]:
        ...
        X = torch.FloatTensor(self.X.todense() if self.is_sparse else self.X)
        library_size = torch.FloatTensor(self.library_size)
        gene_indices = torch.arange(0, self.n_genes, dtype=torch.long)
        result_dict = dict(genes=X, library_size=library_size, gene_indices=gene_indices)
        while count < self.n_epochs:
            count += 1
            yield result_dict
    ...
    def _low_batch_size(self) -> Iterator[Mapping[str, torch.Tensor]]:
        ...
        entry_index = 0
        count = 0
        cell_range = np.arange(self.n_genes)
        if self.shuffle:
            self.rng.shuffle(cell_range)
        while count < self.n_epochs:
            if entry_index + self.batch_size >= self.n_genes:
                count += 1
                batch = cell_range[entry_index:]
                if self.shuffle:
                    self.rng.shuffle(cell_range)
                excess = entry_index + self.batch_size - self.n_genes
                if excess > 0 and count < self.n_epochs:
                    batch = np.append(batch, cell_range[:excess], axis=0)
                    entry_index = excess
                else:
            ...
            if self.is_sparse:
                ...
                genes = torch.FloatTensor(X.todense())
            else:
                genes = torch.FloatTensor(X)
            gene_indices = torch.LongTensor(batch)
            result_dict = dict(genes=genes, library_size=library_size, gene_indices=gene_indices)

            yield result_dict


class GenesThreadedCellSampler(threading.Thread):
    """A wrapped cell sampler for multi-threaded minibatch sampling.
    Attributes:
        sampler: the underlying iterable cell dataset.
        pipeline: the pipeline connecting the sampler (producer) and the caller
    ...
    def __init__(self,
        ...
        super().__init__(daemon=True)
        self.sampler: GenesSampler = GenesSampler(adata, batch_size, n_epochs, rng, shuffle)
        self.pipeline: GenePipeline = GenePipeline()
        self.start()
...
class GenesMultithreadedCellSampler:
    ...
    def __init__(self,
        ...
        self.n_samplers: int = n_samplers
        self.samplers: List[GenesThreadedCellSampler] = [GenesThreadedCellSampler(#List列表 []
            adata,
            batch_size,
