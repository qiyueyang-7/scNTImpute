...
from sklearn.metrics import adjusted_rand_score, normalized_mutual_info_score, silhouette_samples, fowlkes_mallows_score, mutual_info_score, f1_score, davies_bouldin_score
from sklearn.metrics import homogeneity_score, completeness_score, v_measure_score, calinski_harabasz_score, accuracy_score, jaccard_score,rand_score
from sklearn.metrics import adjusted_mutual_info_score, mean_squared_error
from sklearn.metrics.pairwise import cosine_similarity
from scNTImpute.logging_utils import log_arguments, initialize_logger
import psutil
...
def evaluate(adata: ad.AnnData,
    ...
    Returns:
        ...
        with key "ari", "nmi", "asw", "ebm", "k_bet","ri","ami","fmi","mi"and others, respectively. If draw is
    ...
    if cell_type_col and not pd.api.types.is_categorical_dtype(adata.obs[cell_type_col]):
        _logger.warning("scNTImpute.evaluate assumes discrete cell types. Converting cell_type_col to categorical.")
    ...
    if batch_col and not pd.api.types.is_categorical_dtype(adata.obs[batch_col]):
        _logger.warning("scNTImpute.evaluate assumes discrete batches. Converting batch_col to categorical.")
    ...
    if cell_type_col in adata.obs and len(resolutions) > 0 and adata.obs[cell_type_col].nunique() > 1:
        ...
        cluster_key, best_ari, best_nmi, best_ami, best_fmi, best_mi, best_hs, best_cms, best_vms, best_js, best_ac, best_f1, best_cs, best_mse, best_ri = clustering(adata, resolutions=resolutions, cell_type_col=cell_type_col, batch_col=batch_col, clustering_method=clustering_method)
        ...
        asw = np.mean(sw)
        chs = calinski_harabasz_score(adata.X if embedding_key == 'X' else adata.obsm[embedding_key], adata.obs[cell_type_col])
        dbs = davies_bouldin_score(adata.X if embedding_key == 'X' else adata.obsm[embedding_key], adata.obs[cell_type_col])

        _logger.info(f'{embedding_key}_ASW: {asw:7.4f}\t{embedding_key}_CHS: {chs:7.4f}\t{embedding_key}_DBS: {dbs:7.4f}')
        ...
        asw = chs = dbs = cluster_key = best_ari = best_nmi = best_ami = best_fmi = best_mi = best_hs = best_cms = best_vms = best_js = best_ac = best_f1 = best_cs = best_mse =best_ri=None
    ...
    return dict(
        ...
        cms=best_cms,
        vms=best_vms,
        js=best_js, ac=best_ac, f1=best_f1,
        cs=best_cs,
        mse=best_mse,
        chs=chs,
        dbs=dbs,
...
def clustering(
    ...
    best_res, best_ari, best_nmi, best_ami, best_fmi, best_mi, best_hs, best_cms, best_vms, best_js, best_ac,  best_f1, best_cs, best_mse, best_ri= None, -inf, -inf, -inf, -inf, -inf, -inf, -inf, -inf, -inf, -inf, -inf, -inf, -inf, -inf
    for res in resolutions:
        ...
        ri = rand_score(adata.obs[cell_type_col], adata.obs[col])
        nmi = normalized_mutual_info_score(adata.obs[cell_type_col], adata.obs[col])
        ami = adjusted_mutual_info_score(adata.obs[cell_type_col], adata.obs[col])
        fmi = fowlkes_mallows_score(adata.obs[cell_type_col], adata.obs[col])
        mi = mutual_info_score(adata.obs[cell_type_col], adata.obs[col])
        hs = homogeneity_score(adata.obs[cell_type_col], adata.obs[col])
        cms =completeness_score(adata.obs[cell_type_col], adata.obs[col])
        vms = v_measure_score(adata.obs[cell_type_col], adata.obs[col])
        df_zifu_cell_type = adata.obs[cell_type_col]  #
        status_only_dict = df_zifu_cell_type.unique().tolist()
        adata.obs[cell_type_col] = df_zifu_cell_type.apply(lambda x: status_only_dict.index(x))
        js = jaccard_score(adata.obs[cell_type_col].astype(str), adata.obs[col].astype(str), average='micro')
        f1 = f1_score(adata.obs[cell_type_col].astype(str), adata.obs[col].astype(str), average='micro')
        ac = accuracy_score(adata.obs[cell_type_col].astype(str), adata.obs[col].astype(str))
        mse = mean_squared_error(adata.obs[cell_type_col].astype(str), adata.obs[col].astype(str))
        adata.obs[cell_type_col] = adata.obs[cell_type_col].apply(lambda x: status_only_dict[x])
        cs = cosine_similarity(adata.obsm['delta']).mean()
        ...
        if batch_col in adata.obs and adata.obs[batch_col].nunique() > 1:
            ...
            _logger.info(f'Resolution: {res:5.3g}\tARI: {ari:7.4f}\tNMI: {nmi:7.4f}\tRI: {ri:7.4f}\tAMI :{ami:7.4f}\tFMI: {fmi:7.4f}\tMI: {mi:7.4f}\tHS: {hs:7.4f}\tCMS: {cms:7.4f}\tVMS: {vms:7.4f}\tJS: {js:7.4f}\tAC: {ac:7.4f}\tF1: {f1:7.4f}\tCS: {cs:7.4f}\tMSE: {mse:7.4f}\tbARI: {ari_batch:7.4f}\t# labels: {n_unique}')
        else:
            _logger.info(f'Resolution: {res:5.3g}\tARI: {ari:7.4f}\tNMI: {nmi:7.4f}\tRI: {ri:7.4f}\tAMI :{ami:7.4f}\tFMI: {fmi:7.4f}\tMI: {mi:7.4f}\tHS: {hs:7.4f}\tCMS: {cms:7.4f}\tVMS: {vms:7.4f}\tJS: {js:7.4f}\tAC: {ac:7.4f}\tF1: {f1:7.4f}\tCS: {cs:7.4f}\tMSE: {mse:7.4f}\t# labels: {n_unique}')

    return f'{clustering_method}_{best_res}', best_ari, best_nmi, best_ami, best_fmi, best_mi, best_hs, best_cms, best_vms, best_js, best_ac, best_f1, best_cs, best_mse, best_ri
