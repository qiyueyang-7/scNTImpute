...
from scNTImpute.batch_sampler import CellSampler, MultithreadedCellSampler, GenesSampler, GenesMultithreadedCellSampler
from scNTImpute.eval_utils import evaluate
from scNTImpute.models import BaseCellModel, scNTImpute
from scNTImpute.logging_utils import initialize_logger, log_arguments
from scNTImpute.trainers.trainer_utils  import train_test_split, set_seed, _stats_recorder
...
class UnsupervisedTrainer:
    ...
    def __init__(self,
        ...
        lr_decay: float = 6e-5,
        batch_size: int = 2000,
        train_instance_name: str = "scNTImpute",
        restore_epoch: int = 0,
        seed: int = -1,
    ) -> None:
    ...
    def load_ckpt(self, restore_epoch: int, ckpt_dir: Union[str, None] = None) -> None:
        ...
        for attr, fname in self.attr_fname.items():
            ...
            getattr(self, attr).load_state_dict(torch.load(fpath,map_location='cpu'))
    ...
    def update_step(self, jump_to_step: Union[None, int] = None) -> None:
        ...
        Args:
            jump_to_step: the step number to jump to. If None, increment the
                step number by one.
         Args:
            jump_to_step: the step number to jump to. If None, increment the step number by one.
        """
        ...
        **train_kwargs
        ...
        while self.epoch < n_epochs:
            if n_samplers == 1 or self.batch_size >= self.train_adata.n_obs:
                sampler = CellSampler(self.train_adata, self.batch_size, sample_batch_id=self.model.need_batch,
                                      n_epochs=n_epochs - self.epoch, batch_col=batch_col)
            else:
                sampler = MultithreadedCellSampler(self.train_adata, self.batch_size, n_samplers=n_samplers,
                                                   sample_batch_id=self.model.need_batch,
                                                   n_epochs=n_epochs - self.epoch, batch_col=batch_col)

            dataloader = iter(sampler)
            if self.epoch == n_epochs-2:
                new_record, hyper_param_dict, self_scNTImputeadata_X  = self.do_train_step(dataloader,
                n_epochs = n_epochs,
                ...
                **train_kwargs
                ...
                self.train_adata.X = self_scNTImputeadata_X
            else:
            ...
            if self.epoch >= next_ckpt_epoch or self.epoch >= n_epochs:
                ...
                if eval:
                    ...
                    if isinstance(self.model, scNTImpute):
    ...
    def do_train_step(self, dataloader,**kwargs) -> Mapping[str, torch.Tensor]:
        ...
        if self.epoch == kwargs['n_epochs']-2:
            new_record, self_scNTImputeadata_X = self.model.train_step(self.optimizer, data_dict, hyper_param_dict, self.epoch, kwargs['n_epochs'])
            return new_record, hyper_param_dict, self_scNTImputeadata_X
        else:
            new_record = self.model.train_step(self.optimizer, data_dict, hyper_param_dict,
                                                                  self.epoch, kwargs['n_epochs'])
